<template>
  <div class="original-image-container">
    ...
    <div>
      ...
      <p v-if="file">
        upload 이미지 : {{ file.name }} ({{ file.size }} bytes) /
        {{ file.type }}
      </p>
...
export default {
  ...
  methods: {
      ...
      return new Promise((resolve, reject) => {
        ...
        request.onsuccess = (event) => {
          ...
          getAllRequest.onsuccess = (event) => {
            ...
            this.images = event.target.result.map((item) => {
              return "data:image/png;base64," + item.url; // Data URL 형태로 변환
            });

            //console.log("indexedDB에서 불러옴 (디코딩 후):", this.images);
            resolve();
          };
        };
      });
    ...
    // API를 통해 이미지 URL 가져오고 IndexedDB에 저장
    async fetchAndStoreImages() {
      try {
        ...
        if (data.error) {
          ...
          //console.log("데이터", this.images); // 데이터 확인
  ...
  mounted() {
    ...
    // 이미지 가져오기 완료 후 IndexedDB 체크 및 처리
    this.fetchImages().then(() => {
      if (this.images.length === 0) {
        console.log("이미지개수", this.images.length);
        this.fetchAndStoreImages();
      }
    });
  },
};
